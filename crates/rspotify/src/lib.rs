...
//!
//! - `client-reqwest`, TLS available: + `reqwest-default-tls` (reqwest's
//!   default) + `reqwest-rustls-tls` + `reqwest-native-tls` +
//!   `reqwest-native-tls-vendored`
//! - `client-ureq`, TLS available: + `ureq-rustls-tls` (ureq's default)
//!
...
//! values when using methods like
//! [`CredentialsBuilder::from_env`](crate::oauth2::CredentialsBuilder::
//! from_env):
//!
//! ```toml
//! [dependencies]
...
compile_error!(
    "`client-reqwest` and `client-ureq` features cannot both be enabled at the same time, if you \
     want to use `client-ureq` you need to set `default-features = false`"
);
