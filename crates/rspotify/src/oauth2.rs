...
impl Spotify {
    ...
    pub async fn request_client_token(&mut self) -> ClientResult<()> {
        if let Some(_) = self.read_token_cache().await {
            return Ok(());
        }
    ...
    fn get_code_from_user(&self) -> ClientResult<String> {
        ...
        match webbrowser::open(&url) {
            ...
            Err(why) => eprintln!(
                "Error when trying to open an URL in your browser: {:?}. Please navigate here \
                 manually: {}",
                why, url
            ),
        }
...
mod tests {
    ...
    fn test_write_token() {
        ...
        let scope = "playlist-read-private playlist-read-collaborative playlist-modify-public \
