...
use rspotify::model::{AlbumType, Country, Market};
...
pub async fn creds_client() -> Spotify {
    ...
    let creds = CredentialsBuilder::from_env().build().unwrap_or_else(|_| {
        panic!(
            "No credentials configured. Make sure that either the `env-file` feature is enabled, \
             or that the required environment variables are exported (`RSPOTIFY_CLIENT_ID`, \
