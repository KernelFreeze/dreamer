...
use rspotify::model::{Country, Market, RepeatState, SearchType, TimeRange};
...
pub async fn oauth_client() -> Spotify {
    if let Ok(access_token) = env::var("RSPOTIFY_ACCESS_TOKEN") {
        ...
        let creds = CredentialsBuilder::from_env().build().unwrap_or_else(|_| {
            panic!(
                "No credentials configured. Make sure that either the `env-file` feature is \
                 enabled, or that the required environment variables are exported \
        ...
        let scope = "user-read-email user-read-private user-top-read user-read-recently-played \
                     user-follow-read user-library-read user-read-currently-playing \
                     user-read-playback-state user-read-playback-position \
                     playlist-read-collaborative playlist-read-private user-follow-modify \
        ...
        panic!(
            "No access tokens configured. Please set `RSPOTIFY_ACCESS_TOKEN` or \
             `RSPOTIFY_REFRESH_TOKEN`, which can be obtained with the `oauth_tokens` example."
        )
    }
}
