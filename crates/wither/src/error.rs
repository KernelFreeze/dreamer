...
pub enum WitherError {
    ...
    /// An error indicating that an ObjectId is required for the requested
    /// operation.
    #[error("Model must have an ObjectId for this operation.")]
    ModelIdRequiredForOperation,
    /// An error indicating that a model was serialized to a BSON variant other
    /// than a document.
    ...
    /// An error indicating that the server failed to return a document after an
    /// update.
    ...
    /// An error indicating that one of `$set` or `$unset` must be specified for
    /// a migration.
