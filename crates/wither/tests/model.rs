...
async fn model_save_should_save_model_instance_and_add_id() {
    ...
    let precount = User::collection(&db)
    ...
    user.save(&db, None)
        .await
        .expect("Expected a successful save operation.");
    let postcount = User::collection(&db)
        .count_documents(None, None)
        .await
        .unwrap();
...
async fn model_update_should_return_error_with_invalid_update_document() {
    ...
    assert_eq!(
        err.to_string(),
        "An invalid argument was provided to a database operation: update document must have \
         first key starting with '$"
    );
}
...
async fn model_delete_should_delete_model_instance() {
    ...
    let presave = User::collection(&db)
        .count_documents(None, None)
        .await
        .unwrap();
    user.save(&db, None)
        .await
        .expect("Expected a successful save operation.");
    let postsave = User::collection(&db)
        .count_documents(None, None)
        .await
        .unwrap();
    user.delete(&db).await.unwrap();
    let postdelete = User::collection(&db)
        .count_documents(None, None)
        .await
        .unwrap();
...
async fn model_sync_should_modify_indexes_v1_to_v2() {
    ...
    let after_indexes: HashMap<String, IndexModel> = IndexTestV1::get_current_indexes(&db)
        .await
        .expect("error getting current indexes");
    let index_model = after_indexes
        .get("i_-1")
        .expect("Should have index: `i_-1`");
    let index_key_value = index_model
        .keys
        .get("i")
        .expect("Should have key `i`")
